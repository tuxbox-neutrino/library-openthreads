...
Mutex::Mutex() {
	...
	InitializeCriticalSection(&pd->mutex);
...
Mutex::~Mutex() {
	...
	Win32MutexPrivateData* pd = static_cast<Win32MutexPrivateData*>(_prvData);

	DeleteCriticalSection(&pd->mutex);
...
int Mutex::trylock() {
	...
	return TryEnterCriticalSection(&pd->mutex);

}



#else  // spinlock variant
